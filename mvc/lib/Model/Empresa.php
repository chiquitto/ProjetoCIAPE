...
extends Model {
    public function cadastrar(Vo_Empresa $dados) {	
        ...
        else if ($dados->getE_cnpj() == '') {
            throw new Exception('Informe o CNPJ da empresa.', 2);
        }

        else if ($dados->getE_dtFundacao() == '') {
            throw new Exception('Informe a Data de Fundação da empresa.', 3);
        }

        else if ($dados->getE_ramoAtividade() == '') {
            throw new Exception('Informe o Ramo de Atividade da empresa.', 4);
        }

        else if ($dados->getE_endereco() == '') {
            throw new Exception('Informe o Endereço da empresa.', 5);
        }

        else if ($dados->getE_bairro() == '') {
            throw new Exception('Informe o Bairro localiza a empresa.', 6);
        }

        else if ($dados->getE_cep() == '') {
            throw new Exception('Informe o Cep que localiza a empresa.', 7);
        }

        else if ($dados->getE_fone() == '') {
            throw new Exception('Informe o telefone da empresa.', 8);
        }

        else if ($dados->getE_email() == '') {
            throw new Exception('Informe o Email da empresa.', 9);
        }      

        else if ($dados->getM_concorrenteA() == '') {
            throw new Exception('Informe os concorrentes da empresa.', 10);
        }
        
        

        else if ($dados->getM_atendimento() != 1 || $dados->getM_estacionamento() != 1 || $dados->getM_preco() != 1 || $dados->getM_variedade() != 1 || $dados->getM_espacoFisico() != 1 || $dados->getM_localizacao() != 1 || $dados->getM_qualidade() != 1 || $dados->getM_outros() != 1 || $dados->getM_descricaoOutrosPontosFortes() == '') {
            throw new Exception('2.4 Informe os seus pontos fortes em relação a concorrência.', 11);
        }

        else if ($dados->getM_mAtendimento() != 1 || $dados->getM_mEstacionamento() != 1 || $dados->getM_mPreco() != 1 || $dados->getM_mVariedade() != 1 || $dados->getM_mEspacoFisico() != 1 || $dados->getM_mLocalizacao() != 1 || $dados->getM_mQualidade() != 1 || $dados->getM_mOutros() != 1 || $dados->getM_mOutrosPontosDescricao() == '') {
            throw new Exception('2.5 Informe os seus pontos para melhoria em relação a concorrência.', 12);
        }

        else if ($dados->getM_promocao() == '') {
            throw new Exception('2.6 Informe as promoções da empresa.', 13);
        }

        

        else if (($dados->getM_radio() != 1) || ($dados->getM_jornal() != 1) || ($dados->getM_panfleto() != 1) || ($dados->getM_outrosMeiosComunicacao() != 1) || ($dados->getM_outrosMeiosComunicacaoDesc() == '')) {
            throw new Exception('2.7 Informe os meios que utiliza.', 14);
        }

        
        else if ($dados->getP_formacaoPreco() == '') {    
            throw new Exception('3.1 Informe como forma os preços de seus produtos.', 15);
        }

        else if ($dados->getP_precoConcorrencia() == '') {
            throw new Exception('3.2 Informe como estão seus preços em relação a concorrência .', 16);
        }

        else if ($dados->getV_seg() != 1 || $dados->getV_ter() != 1 || $dados->getV_qua() != 1 || $dados->getV_qui() != 1 || $dados->getV_sex() != 1 || $dados->getV_sab() != 1 || $dados->getV_dom() != 1) {
            throw new Exception('4.2 Informe os dias da semana de maior movimento.', 17);
        }

        if ($dados->getV_manha() != 1 || $dados->getV_tarde() != 1 || $dados->getV_noite() != 1) {
            throw new Exception('4.3 Informe os horários de maior movimento.', 18);
        }
        ...
        if ($dados->getV_cheque() != 1 || $dados->getV_ticket() != 1 || $dados->getV_cartao() != 1 || $dados->getV_avista() != 1 || $dados->getV_cardeneta() != 1 || $dados->getV_outraFormaPagto() != 1 || $dados->getV_outraFormaPagtoDesc() == '') {
            throw new Exception('4.4 Informe a forma de pagamento oferecidas aos clientes.', 19);
        }*/
        //------------------
